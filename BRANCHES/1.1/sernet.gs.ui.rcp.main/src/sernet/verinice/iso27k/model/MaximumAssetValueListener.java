...
public class MaximumAssetValueListener implements ILinkChangeListener,
    ...
    public void determineIntegritaet(CascadingTransaction ta)
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("Determining integrity for " + sbTarget);
        }
        ...
        if (!sbTarget.getSchutzbedarfProvider().isCalculatedIntegrity()) {
            if (LOG.isDebugEnabled()) {
                LOG.debug("Keeping current integrity " + sbTarget.getSchutzbedarfProvider().getIntegritaet() + " for " + sbTarget.getTitle());
            }
            return;
        }
        
        if (LOG.isDebugEnabled()) {
            LOG.debug("Using maximum integrity " + highestValue + " for " + sbTarget.getTitle());
        }
    ...
    public void determineVerfuegbarkeit(CascadingTransaction ta)
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("Determining availability for " + sbTarget);
        }
        ...
        if (!sbTarget.getSchutzbedarfProvider().isCalculatedAvailability()) {
            if (LOG.isDebugEnabled()) {
                LOG.debug("Keeping current availability " + sbTarget.getSchutzbedarfProvider().getVerfuegbarkeit() + " for " + sbTarget.getTitle());
            }
            return;
        }
        
        if (LOG.isDebugEnabled()) {
            LOG.debug("Using maximum availability " + highestValue + " for " + sbTarget.getTitle());
        }
    ...
    public void determineVertraulichkeit(CascadingTransaction ta)
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("Determining confidentiality for " + sbTarget);
        }
        ...
        if (!sbTarget.getSchutzbedarfProvider().isCalculatedConfidentiality()) {
            if (LOG.isDebugEnabled()) {
                LOG.debug("Keeping current confidentiality " + sbTarget.getSchutzbedarfProvider().getVertraulichkeit() + " for " + sbTarget.getTitle());
            }            
            return;
        }
        
        if (LOG.isDebugEnabled()) {
            LOG.debug("Using maximum confidentiality " + highestValue + " for " + sbTarget.getTitle());
        }
